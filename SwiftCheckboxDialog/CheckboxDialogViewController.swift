...
enum DialogCheckboxViewEnum {
    case countries
}

protocol CheckboxDialogViewDelegate : class {
    func onCheckboxPickerValueChange(_ component: DialogCheckboxViewEnum, values: TranslationDictionary)
}

class CheckboxDialogViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    var tableData: [(name: String, translated: String)] = [] // all values in the table
    var defaultValues: [(name: String, translated: String)] = [] // values loaded from user defaults and shouldn't be change in this class (used on cancel selection and restoring preselected values)
    var componentName: DialogCheckboxViewEnum?
    weak var delegateDialogTableView: CheckboxDialogViewDelegate?
